...
from model.mcr import BertPretrain
...
def parse_args():
    ...
    parser.add_argument(
        "--label-prop",default='true', action="store_true", help="use label propagation."
    )
    parser.add_argument(
        "--sentence-patch-sim",
        action="store_true",
    ...
    parser.add_argument("--use-ssl",default='true', action="store_true", help="use SSL.")
    ...
    parser.add_argument("--ssl_loss", type=str, default="contrastive", help="fro/contrastive")
    parser.add_argument(
        "--image_features_type",
        type=str,
        default="faster_rcnn",
    ...
    parser.add_argument("--use-ema", default='true',action="store_true", help="use EMA model")
    parser.add_argument(
        "--bbox-reg",default='true', action="store_true", help="use bbox regression loss"
    )
    parser.add_argument(
        "--trans-func",
        action="store_true",
    ...
    parser.add_argument(
        "--model_config",default='configs/mcr_config.json', type=str, help="path to model structure config json"
    )
    ...
    parser.add_argument(
        "--use-phrase-mask", default='true',action="store_true", help="use phrase specific masking."
    )
    parser.add_argument(
        "--encoding-type",
        type=str,
...
def evaluate(test_loader, model, device, args):
    ...
    for (
        ...
        object_label_input_ids = object_label_input_ids.squeeze(-2)  # 8*100*16
        object_label_attention_mask = object_label_attention_mask.squeeze(-2)  # 8*100*16
        object_label_input_ids = object_label_input_ids.view(-1, 16)  # 800*16
        object_label_attention_mask = object_label_attention_mask.view(-1, 16)  # 800*16
        prompt_input_ids = prompt_input_ids.squeeze(-2)
        prompt_attn_mask = prompt_attn_mask.squeeze(-2)
        prompt_input_ids = prompt_input_ids.view(-1, 16)
        prompt_attn_mask = prompt_attn_mask.view(-1, 16)
        if int(num_query[0]) > 0:
            ...
            unique_sequences = []
            [unique_sequences.append(x) for x in argmin_cos_dist if x not in unique_sequences]
            argmin_cos_dist = unique_sequences

            # # 计算每个元组的出现次数
            # tuple_counts = Counter(tuple(item) for item in argmin_cos_dist)
            #
            # # 过滤出现次数大于1的元组，并转换回列表
            # duplicates = [list(item) for item, count in tuple_counts.items() if count > 1]
            #
            # print("重复的元素有：")
...
def evaluate_helper(image_id, query_box_max_id, phrase_queries, num_queries):
    for img_id, n_query in zip(image_id, num_queries):
        ...
        out_file = os.path.join(ref_dir, "test", str(img_id) + ".json")
        # 检查文件是否存在
        if not os.path.exists(out_file):
            # 如果文件不存在，就创建文件并写入初始数据
            file = open(out_file, "w")
            data = json.dump(gold, file, indent=4)
            file.close()
...
if __name__ == "__main__":
    ...
    wordEmbedding = data_utils.load_vocabulary("datasets/glove.6B.300d.txt")
