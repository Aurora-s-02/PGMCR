...
from model.bert_layers import BertLayer
from model import bbox_regression
# from model import hungarian

from model.losses import ContrastiveLoss, fro_norm
import torch
...
class ImageEmbedder(nn.Module):
    def __init__(self, config, img_dim, obj_dim):
        ...
        self.prompt_linear = nn.Linear(obj_dim, config["hidden_size"])
        ...
        self.prompt_norm = nn.LayerNorm(config["hidden_size"], eps=1e-12)
    ...
    def forward(self, img_feat, img_pos_feat, prompt_embeds, object_label_embeds, img_masks=None):
        ...
        transformed_prompt = self.prompt_norm(
            self.prompt_linear(prompt_embeds)
        )
        ...
        loc_emb_cat = torch.cat( # 8*100*80
        ...
        embeddings = transformed_im + transformed_pos + transformed_prompt + transformed_obj
...
class BertPretrain(nn.Module):
    ...
    def cosine_similarity_matrix(self, matrix):
        ...
        norm = torch.norm(matrix, p=2, dim=-1, keepdim=True) # 计算欧几里得范数
        matrix = matrix / norm
    ...
    def random_mask(
        ...
        if masked_indices is None:
            masked_indices = torch.bernoulli(probability_matrix).bool() # 伯努利抽样
    ...
    def forward(
        ...
        # Encode the test embedings using test encoder
        ...
        prompt_output = self.text_encoder.bert(
            prompt_input_ids,
            attention_mask=prompt_attn_mask,
            return_dict=True,
            mode="text",
        )
        prompt_embeds = prompt_output.last_hidden_state
        prompt_embeds = torch.mean(prompt_embeds, dim=-2)
        prompt_embeds = prompt_embeds.view(
            image_features.shape[0], image_features.shape[1], -1
        )

        image_embeds = self.visual_embedder(
            image_features, object_regions, prompt_embeds, object_label_embeds
        )
        ...
        phrase_embeddings = torch.mean(phrase_embeddings, dim=-2) # 8*128*768
        if self.adaptor_layers:
