...
def parse_args():
    ...
    parser.add_argument("--gpus", type=int, default=1, help="training epochs")
    parser.add_argument(
        "--save_name",
        type=str,
        default="solve/model",
...
if __name__ == "__main__":
    ...
    wordEmbedding = data_utils.load_vocabulary("datasets/glove.6B.300d.txt") # 加载词向量 词向量维度300  ints_to_objs:{num:word}  objs_to_ints:{word:num}
    ...
    train_dset = ( # train  image_id是训练集的图片id
        localized_narratives_pretrain_loader.LocalizedNarrativesFlickr30dataset(
            dataroot="datasets/",
            image_features_type=args.image_features_type,
            word_embedding=wordEmbedding,
    ...
    ssl_dset = localized_narratives_pretrain_loader.LocalizedNarrativesFlickr30dataset( # train  image_id是测试集的图片id
        dataroot="datasets/",
        image_features_type=args.image_features_type,
        word_embedding=wordEmbedding,
        split="train",
    ...
    save_path = os.path.join("saved", args.save_name, "model" + ".pt")
