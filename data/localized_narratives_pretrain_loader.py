...
NoneType = type(None)
bert_version = r"bert-base-uncased"
...
class LocalizedNarrativesFlickr30dataset(Dataset):
    def __init__(
        ...
        self.localized_narratives_annotated_testval_file = os.path.join(
            dataroot,
            "cin_annotations/testval_annotations_new.json",
        )
    ...
    def __getitem__(self, index):
        ...
        if int(image_id) in self.train_image_ids or self.split == "test":
            ...
            scaled_bboxes = bboxes / hw_array # 归一化
            scaled_bboxes = scaled_bboxes.tolist()
            bboxes = bboxes.tolist()
            labels = self.obj_detection_dict[image_id]["classes_2"]
            prompt_input_ids = self.obj_detection_dict[image_id]["prompt_input_ids"]
            prompt_attn_mask = self.obj_detection_dict[image_id]["prompt_attn_mask"]
            object_label_input_ids = self.obj_detection_dict[image_id]["object_label_input_ids"]
            object_label_attn_mask = self.obj_detection_dict[image_id]["object_label_attn_mask"]
            attrs = (
            ...
            scaled_bboxes = scaled_bboxes.tolist()
            bboxes = bboxes.tolist()
            labels = self.val_obj_detection_dict[image_id]["classes_2"]
            prompt_input_ids = self.val_obj_detection_dict[image_id]["prompt_input_ids"]
            prompt_attn_mask = self.val_obj_detection_dict[image_id]["prompt_attn_mask"]
            object_label_input_ids = self.val_obj_detection_dict[image_id]["object_label_input_ids"]
            object_label_attn_mask = self.val_obj_detection_dict[image_id]["object_label_attn_mask"]
            ...
            height = bboxes[..., 3] - bboxes[..., 1]
            width = bboxes[..., 2] - bboxes[..., 0]
            bboxes = torch.cat( # 将area加入bboxes
                (
                    bboxes,
                    area.unsqueeze_(-1),
                ),
        ...
        if self.split == "train":
            if int(image_id) in self.train_image_ids:
                ...
                org_entities = self.localized_narratives_testval_data[phrase_file[0]][ # query
                    "query"
                ]
        ...
        while len(sense2vec_feats) < self.max_caption_length:
            sense2vec_feats.append(torch.tensor([0.0] * 128))
        pad_width = self.max_queries - num_query # 填补的query数量
        ...
        if self.split == "train":
            if self.sentence_patch_sim:
                return (
                    ...
                    phrase_queries,
                    feature,
                    torch.tensor(object_label_input_ids),
                    torch.tensor(object_label_attn_mask),
                    torch.tensor(prompt_input_ids),
                    torch.tensor(prompt_attn_mask),
                    bboxes,
